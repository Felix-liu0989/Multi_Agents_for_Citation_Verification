...
ipts = json.load(
    open("scholar_copilot_eval_data_Remove_Duplicates.json",encoding='utf-8')
)

cite_model = 'deepseek-chat'
save_name = cite_model.replace('-','_')
fout_path = f'{save_dir}/tmp/{save_name}_0616.jsonl'
...
need_list = [x for x in ipts[1:61] if x['idx'] not in s][:]
...
def process_with_iterations_v2(js):
    ...
    if 'related_works_gpt4o_mini' in js and js['related_works_gpt4o_mini']:
        ...
        win_rate_response2 = evaluation_client.get_completion(
            win_rate_prompt2
        )
