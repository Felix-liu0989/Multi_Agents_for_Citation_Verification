...
topic_cls_system_instruction = """You are a helpful 2-label classification assistant 
which helps me label papers based on their specific research directions or technical categories. 
"""
...
def topic_cls_main_prompt(paper,topics):
    json_format_fields = []
    for topic in topics:
        json_format_fields.append(f'  "{topic}": <return True if the paper is relevant to {topic}, False otherwise>')
    
    json_format = "{\n" + ",\n".join(json_format_fields) + "\n}"
